...
public class LearnController implements Serializable {
    ...
    public InitialisationError doInitializeQueues(Deck deck){
        this.cardsQueue.addAll(learnService.findCardsToLearn(deck.getContent(), currentUser));
        this.neverLearnedCardsQueue.addAll(learnService.findNeverLearnedCards(deck.getContent(), currentUser));
    ...
    public Card doGetNextCard(){
        if(cardsQueue.isEmpty()){
            return neverLearnedCardsQueue.poll();
        }
